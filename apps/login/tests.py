...
from  apps.render.models import VideoRender,DataTextVideo,video_url,Count_Use_data
from apps.home.models import Folder, ProfileChannel, Font_Text, Voice_language, syle_voice
from apps.login.models import CustomUser


def get_folder_data_from_cache():
    folder_data = cache.get('Folder_cache')
    if folder_data is not None:
        return folder_data
    else:
        # Nếu cache không có dữ liệu (do bị xóa hoặc hết hạn), load lại từ database và lưu vào cache
        all_data = Folder.objects.all()
        cache.set('Folder_cache', all_data)
        return all_data


def get_profile_data_from_cache():
    profile_data = cache.get('ProfileChannel_cache')
    if profile_data is not None:
        return profile_data
    else:
        all_data = ProfileChannel.objects.all()
        cache.set('ProfileChannel_cache', all_data)
        return all_data

def get_voice_language_data_from_cache():
    voice_language_data = cache.get('Voice_language_cache')
    if voice_language_data is not None:
        return voice_language_data
    else:
        all_data = Voice_language.objects.all()
        cache.set('Voice_language_cache', all_data)
        return all_data
    
def get_syle_voice_data_from_cache():
    syle_voice_data = cache.get('syle_voice_cache')
    if syle_voice_data is not None:
        return syle_voice_data
    else:
        all_data = syle_voice.objects.all()
        cache.set('syle_voice_cache', all_data)
        return all_data
...
def get_video_render_data_from_cache():
    ...
    if video_render_data is not None:
        ...
        all_data = VideoRender.objects.all()
        cache.set('VideoRender_cache', all_data)
        return all_data

def get_data_text_video_data_from_cache():
    data_text_video_data = cache.get('DataTextVideo_cache')
    if data_text_video_data is not None:
        return data_text_video_data
    else:
        all_data = DataTextVideo.objects.all()
        cache.set('DataTextVideo_cache', all_data)
        return all_data

def get_video_url_data_from_cache():
    video_url_data = cache.get('video_url_cache')
    if video_url_data is not None:
        return video_url_data
    else:
        all_data = video_url.objects.all()
...
class setting(LoginRequiredMixin, TemplateView):
    ...
    def set_form_choices(self, request ,form, profiles, profile):
        ...
        folder_data = get_folder_data_from_cache()
        syle_voice_data = get_syle_voice_data_from_cache()

        if request.user.is_superuser:
            form.fields['folder_name'].queryset = folder_data.all()
            form.fields['folder_name_seting'].queryset = folder_data.all()
            form.fields['forder_setting'].queryset = folder_data.all()
            form.fields['channel_forder_name_setting'].queryset = folder_data.all()
        else:
            form.fields['folder_name'].queryset = folder_data.filter(use=request.user)
            form.fields['folder_name_seting'].queryset = folder_data.filter(use=request.user)
            form.fields['forder_setting'].queryset = folder_data.filter(use=request.user)
            form.fields['channel_forder_name_setting'].queryset = folder_data.filter(use=request.user)

        if profile is None:
            voice = syle_voice_data.filter(voice_language=1)
        else:
            voice = syle_voice_data.filter(voice_language=profile.channel_voice)
    ...
    def get(self, request):
        ...
        print(folder_data)
        if request.user.is_superuser:
            folder = folder_data[0] if folder_data else None
        else:
            folder = next((item for item in folder_data if item.get('use') == request.user),, None)
    ...
    def post(self, request):
        ...
        if request.user.is_superuser:
            ...
            form.fields['folder_name'].queryset = Folder.objects.filter(use=request.user)
            form.fields['folder_name_seting'].queryset = Folder.objects.filter(use=request.user)
            form.fields['forder_setting'].queryset = Folder.objects.filter(use=request.user)
            form.fields['channel_forder_name_setting'].queryset = Folder.objects.filter(use=request.user)
