...
def get_video_info(url):
    ...
    data = response.json()["api"]["mediaItems"]


    for iteam in data:
        if iteam["mediaRes"] == "1280x720":
            title = response.json()["api"]['title']
            preview_url = iteam['mediaPreviewUrl']
            image_preview_url = iteam['mediaThumbnail']
    # In ra kết quả
    print(title)
    print(preview_url)
    print(image_preview_url)
