...
SERVER='http://157.90.208.177:5085'
...
@shared_task(bind=True, priority=10,name='render_video_reupload',time_limit=7200,queue='render_video_reupload')
        ...
        if data.get('channel_music_active'):
            ...
            if audio_duration:
                ...
                ffmpeg_bgm_command = [
                    ...
                    '-filter_complex', f"[1]atrim=start={start_time}:duration={audio_duration}[bgm];[0][bgm]amix=inputs=2:duration=first",
...
def get_voice_chat_ai_human(data, text, file_name):
    ...
    payload = {
        "model_name": data.get("style_name"),
        "emotion": "neutral",
        "language": "ko",
        "pitch": 10,
        "text": text,
