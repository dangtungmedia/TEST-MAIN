...
@shared_task(bind=True, priority=0,name='apps.render.task.render_video',time_limit=7200)
...
def get_text_lines(data, text,width=1920):
    ...
    wrapped_text = ""
    font = data['font_name']
    font_text = find_font_file(font, r'font')
    ...
    try:
        ...
        if language == 'Japanese-VoiceVox':
            ...
            with open(f'media/{video_id}/input_files.txt', 'w') as file:
                for text_entry in text_entries:
                    file_name = f'media/{video_id}/voice/{text_entry["id"]}.wav'
                    get_voice_chat_gpt(data, text_entry['text'], file_name)
                    processed_entries += 1
                    percent_complete = (processed_entries / total_entries) * 100
                    update_status_video(f"Đang Render : Đang tạo giọng đọc {percent_complete:.2f}%", data['video_id'], task_id, worker_id)
                    file.write(f"file 'voice/{text_entry['id']}.wav'\n")
...
def get_voice_chat_gpt(data, text, file_name):
    ...
    if not os.path.exists(directory):
        os.makedirs(directory, exist_ok=True)
    url = f"https://api.ttsopenai.com/api/v1/public/text-to-speech-stream"
    data ={
    "model": "tts-1",
    "speed": 1,
    "input": text,
...
def download_image(data, task_id, worker_id):
    ...
    for image in images:
        ...
        for attempt in range(30):  # Thử tải lại 30 lần nếu thất bại
            try:
                ...
                if response.status_code == 200:
                    ...
                    update_status_video(
                        f"Render Lỗi : Trạng thái không thành công - {response.status_code} - {url}",
                        video_id, task_id, worker_id
                    )
            except requests.RequestException as e:
                print(f"Lỗi yêu cầu khi tải xuống {url}: {e}")
                update_status_video(
                    f"Render Lỗi : Lỗi yêu cầu - {e} - {url}",
                    video_id, task_id, worker_id
                )
            except Exception as e:
        ...
        if not image_downloaded:
            success = False
            update_status_video(
                f"Đang Render : Không thể tải xuống hình ảnh",
                video_id, task_id, worker_id
            )

    percent_complete = (downloaded_images / total_images) * 100
    update_status_video(
        f"Đang Render : Hoàn thành tải xuống hình ảnh ({downloaded_images}/{total_images}) - {percent_complete:.2f}%",
        video_id, task_id, worker_id
    )
    return success
