...
def render_video(self, data):
    ...
    success = text_to_audio(data, task_id, worker_id)
    if not success:
        # shutil.rmtree(f'media/{video_id}')
        return
...
def process_voice_entry(data, text_entry, video_id, task_id, worker_id, language):
    ...
    if not success:
        ...
        update_status_video(
                            f"Đang Render : Không thể tạo giọng nói cho đoạn văn bản ID {text_entry['id']}",
...
def text_to_audio(data, task_id, worker_id):
    try:
