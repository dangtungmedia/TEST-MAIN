...
if __name__ == "__main__":
    ...
    if public_ip == "210.245.74.61":
        ...
        if local_ip:
            ...
            os.system(f"celery -A celeryworker worker -l INFO --hostname={local_ip}-Reup --concurrency=8 -Q render_video_content,render_video_reupload --prefetch-multiplier=1")
    elif public_ip == "27.72.153.24":
        os.system(f"celery -A celeryworker worker -l INFO --hostname={public_ip}-Reup --concurrency=5 -Q render_video_content,render_video_reupload --prefetch-multiplier=1")
    else:
